...
export default function GuestsSection({ fieldClass }) {
    const guests = useEditBookingStore((state) => state.form.guests);
    const updateGuestField = useEditBookingStore((state) => state.updateGuestField);
    const addGuest = useEditBookingStore((state) => state.addGuest);
    const removeGuest = useEditBookingStore((state) => state.removeGuest);
    ...
    return (
        <div className="md:col-span-2 space-y-4">
            ...
            {guests.map((guest, index) => (
                <div key={index} className="grid grid-cols-1 gap-3 rounded-xl border border-base-300/60 p-4 md:grid-cols-3">
                    <div>
